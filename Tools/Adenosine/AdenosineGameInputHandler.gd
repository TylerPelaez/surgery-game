...
const offset = Vector2(-170, -300)

const injection_zone_min_x = -offset.x
const injection_zone_max_x = 1500
const injection_zone_min_y = -offset.y + 40
const injection_zone_max_y = 1000
...
func _draw():
	if Utils.DRAW_DEBUG:
		draw_circle(Vector2(injection_zone_min_x, injection_zone_min_y), 100, Color.white)
		draw_circle(Vector2(injection_zone_min_x, injection_zone_max_y), 100, Color.white)
		draw_circle(Vector2(injection_zone_max_x, injection_zone_min_y), 100, Color.white)
		draw_circle(Vector2(injection_zone_max_x, injection_zone_max_y), 100, Color.white)
	...
	if !fill_syringe_stage:
		if $Syringe/Plunger.position.y > plunger_starting_pos.y:
			...
			if in_injection_zone:
				...
				$Success.play()
				var input_data = AdenosineToolInputData.new()
				input_data.initialize(null)
		...
		if Input.is_action_just_pressed("lmb"):
			...
			$Syringe.position = Vector2(-offset.x + get_mouse_pos().x - 360, -offset.y + get_mouse_pos().y - 675)
